...
async def async_setup_entry(hass: HomeAssistant, entry):
    ...
    async def pick_random_workout(call: ServiceCall):
        ...
        # Parse the config to find the matching URL
        ...
        async with session.get(target_url) as response:
            ...
            # Fire event to update the specific sensor
            hass.bus.async_fire(f"{DOMAIN}_update_{category_target}", {
                "title": workout.get("title"),
                "video_url": workout.get("url"),
                "video_id": workout.get("url").split("v=")[-1] if "v=" in workout.get("url") else ""
            })
